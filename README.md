...
최저 54 프레임 -> 보스 몬스터의 원거리 공격, 플레이어 피격의 Screen Effect, 다수의 몬스터 및 투사체 동시 출현 시 프레임 저하 확인.

최저 프레임을 더 높이기 위해서는 모든 투사체를 불투명 방식으로 교체하고, 몬스터의 사망 이펙트 교체가 필요

최고 84 프레임

아래 사진에서 급격한 프레임 저하로 보이는 부분은 실제 프레임 드랍 및 스터터링 현상이 아닌, Standalone VR 프레임 측정을 위한 어플리케이션 문제로 인하여 18프레임으로 고정된 상황입니다.

실제 상단 유튜브 링크에 업로드된 플레이 영상의 프레임 촬영 영상입니다.
